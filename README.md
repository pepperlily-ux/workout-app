...
**⚠️ Claude에 대한 필수 지시사항:**
사용자가 **"푸시해", "배포해", "커밋해"** 등의 명령을 내리면, Claude는 **반드시 먼저** 이 섹션을 읽고 체크리스트를 따라야 합니다.
...
- [ ] **2단계: JavaScript 에러 검증** - 새로운 state/함수를 사용하는 경우 선언 여부 확인
  - useState로 state 선언했는지 확인
  - 사용하는 컴포넌트에 필요한 state가 모두 있는지 확인
  - onClick, onChange 등 이벤트 핸들러 함수가 정의되어 있는지 확인
- [ ] **3단계: 버그 없는지 확인** - localStorage 규칙 위반, 기존 기능 손상 여부 확인
- [ ] **4단계: SW 버전 +1 증가** ← **이걸 먼저!**
  - `sw.js` 파일의 `CACHE_NAME` 버전을 **정확히 1 증가**
  ```javascript
  // 예: const CACHE_NAME = 'metamong-v62'; → 'metamong-v63';
  ```
- [ ] **5단계: 스플래시 화면 버전 텍스트 업데이트**
  - `index.html`의 스플래시 화면 버전을 같은 버전으로 업데이트
  ```html
  <!-- 예: <div class="splash-version">sw ver.62</div> → sw ver.63 -->
  ```
- [ ] **6단계: 사용자 확인** - 변경 사항을 사용자에게 설명하고 **푸시 허락을 받기**
- [ ] **7단계: 커밋** - 의미있는 커밋 메시지 작성
- [ ] **8단계: 푸시** - 원격 저장소에 푸시

**중요 사항:**
- 로컬 개발 중에는 Service Worker 버전을 절대 변경하지 마세요. 오직 **"지금 푸시해" 명령을 받았을 때만** 버전을 1 증가시킵니다.
- 이 앱은 **실제 사용자가 사용 중인 프로덕션 앱**입니다. 테스트되지 않은 코드를 배포하면 앱이 멈출 수 있습니다.
- 긴급 상황이 아니라면 성급한 롤백을 하지 마세요. 진짜 원인을 먼저 파악하세요.
- **절대 사용자 허락 없이 푸시하지 마세요.** 긴급 상황이라도 사용자에게 먼저 알리고 승인을 받아야 합니다.
